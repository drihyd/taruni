...
<form id="crudTable" action="{{url(Config::get('constants.general').'/departments/update')}}" method="POST"  enctype="multipart/form-data">
<input type="hidden" name="id" value="{{$departments_data->id}}">
@else
<form id="crudTable" action="{{url(Config::get('constants.general').'/departments/store')}}" method="POST"  enctype="multipart/form-data" data-parsley-validate>
@endif  
      @csrf
      <div class="row">
        <div class="col-md-5">
      ...
      <a href="{{url(Config::get('constants.general').'/departments')}}" class="btn btn-info">Back</a>
    </div>
    <div class="col-md-5">
